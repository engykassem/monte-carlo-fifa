...
def main():
    ...
    # Store original teams for SMS display
    original_teams = teams.copy()
    
    # Pad teams to 4 if less than 4 (for simulation purposes only)
    if len(teams) < 4:
    ...
    if job_index >= len(original_teams):
        print(f"Worker {job_index} has no team assigned. Exiting.")
        return
    
    my_team = original_teams[job_index]
    ...
    if mode == "group_stage":
        ...
        # Filter results to only show original teams
        filtered_results = {team: results[team] for team in original_teams}
        body = format_group_stage_sms(my_team, original_teams, filtered_results)
    else:
        print(f"Worker {job_index}: Running TOURNAMENT simulation for {my_team}...")
        results = run_tournament_monte_carlo(teams)
        # Filter results to only show original teams
        filtered_results = {team: results[team] for team in original_teams}
        body = format_tournament_sms(my_team, original_teams, filtered_results)
